// mintows Browser Relay v0.50 - signaling/state relay only; WebRTC carries media directly.
...
const TURN_CONFIGURED=TURN_URLS.length>0&&!!TURN_USER&&!!TURN_CRED;
...
const server=http.createServer(async(req,res)=>{
  ...
  if(u.pathname==='/ice' && req.method==='POST'){
    ...
    try{
      ...
      if(ICE_POLICY==='relay' && !TURN_CONFIGURED){res.writeHead(503,{'content-type':'application/json','cache-control':'no-store'});return res.end(JSON.stringify({ok:false,error:'TURN is not configured'}));}
      const iceServers=[{urls:'stun:stun.l.google.com:19302'},{urls:'stun:stun.cloudflare.com:3478'}];
      if(TURN_CONFIGURED) iceServers.push({urls:TURN_URLS.length===1?TURN_URLS[0]:TURN_URLS,username:TURN_USER,credential:TURN_CRED});
      res.writeHead(200,{'content-type':'application/json','cache-control':'no-store'});
      return res.end(JSON.stringify({ok:true,iceTransportPolicy:ICE_POLICY,iceServers,turnConfigured:TURN_CONFIGURED,version:'0.49'}));
    }catch(e){res.writeHead(400,{'content-type':'application/json'});return res.end(JSON.stringify({ok:false,error:'bad request'}));}
  }
  if(u.pathname==='/health'){res.writeHead(200,{'content-type':'application/json'});return res.end(JSON.stringify({ok:true,service:'mintows Browser Relay',version:'0.49',transport:'WebRTC',agentConnected:!!agent,clients:clients.size,agentConnectedAt,agentLastMessageAt,agentAuthFailures,clientAuthFailures,gateConfigured:!!GATE,gateLength:GATE.length,turnConfigured:TURN_CONFIGURED,turnUrlCount:TURN_URLS.length,iceTransportPolicy:ICE_POLICY}))}
...
wss.on('connection',(ws,req)=>{const role=ws._mintowsRole;console.log(`[${role.toUpperCase()}] connection`,req.socket.remoteAddress||'unknown');if(role==='agent'){let authed=false;ws.on('message',raw=>{try{const m=JSON.parse(raw.toString());if(!authed){if(m.type!=='auth'||m.role!=='agent'||!GATE||String(m.password||'')!==GATE){agentAuthFailures++;send(ws,{type:'auth-error',error:'agent authentication failed'});return ws.close(1008,'authentication failed')}authed=true;if(agent&&agent!==ws)try{agent.close(4001,'replaced')}catch{}agent=ws;agentConnectedAt=new Date().toISOString();send(ws,{type:'auth-ok'});console.log('[AGENT] authenticated');return}agentLastMessageAt=new Date().toISOString();if(m.type==='state')for(const c of clients.values())send(c,m);else if(m.type==='webrtc-signal'){const cid=String(m.clientId||'');const c=clients.get(cid);console.log(`[AGENT] signal -> client=${cid} kind=${m.data?.kind||''}`);if(c)send(c,{type:'webrtc-signal',data:m.data})}else if(['file-chooser','file-upload-progress','file-uploaded','file-upload-error','file-download-start','file-download-chunk','file-download-end'].includes(m.type))for(const c of clients.values())send(c,m)}catch(e){console.warn('[AGENT] message error',e.message||e)}});ws.on('close',()=>{if(agent===ws)agent=null});return}
let authed=false;ws._clientId=require('crypto').randomUUID();ws.on('message',raw=>{try{const m=JSON.parse(raw.toString());if(!authed){if(m.type!=='auth'||m.role!=='client'||!GATE||String(m.password||'')!==GATE){clientAuthFailures++;send(ws,{type:'auth-error',error:'ゲートパスワードが正しくありません'});return ws.close(1008,'authentication failed')}authed=true;clients.set(ws._clientId,ws);send(ws,{type:'auth-ok',clientId:ws._clientId});if(agent)send(agent,{type:'state-request'});console.log('[CLIENT] authenticated',ws._clientId);return}if(m.type==='webrtc-signal'){console.log(`[CLIENT] signal -> agent client=${ws._clientId} kind=${m.data?.kind||''}`);if(agent)send(agent,{type:'webrtc-signal',clientId:ws._clientId,data:m.data});return}if(agent&&agent.readyState===WebSocket.OPEN)send(agent,{type:'input',data:m});else send(ws,{type:'relay-error',error:'Windows Browser agent is offline'})}catch(e){console.warn('[CLIENT] message error',e.message||e)}});ws.on('close',()=>{clients.delete(ws._clientId);if(agent)send(agent,{type:'webrtc-client-left',clientId:ws._clientId})});});
server.listen(PORT,'0.0.0.0',()=>console.log(`mintows Browser Relay v0.50 listening on ${PORT}`));
